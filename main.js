...
class App extends Component{
   ...
   findingIndex = () => {
    ...
    switchChange = () => {
      const { switchChange } = this.state;
      switchChange == 'UnHighlight' ? this.setState({ switchChange : 'Highlight' }) : this.setState({ switchChange : 'UnHighlight' });
    }
   ...
   highlight = (colour) => {
     let  selectedtext = this.selectedText();
      var replceText = "<span style='background:#0f81ce' >"+selectedtext+"</span>";
      var gethtmlText = textToHighlight;
       replcedtext = gethtmlText.replace(selectedtext ,  replceText);
      textToHighlight = replcedtext ;
   }
   ...
   render(){
      let checkarray = [1];
      const { enableHighlighter , switchChange }= this.state;
      return(
         <div>
            {false && <Button type="primary" onClick = {this.findingIndex}>Text Highlight</Button> }
            <Switch style={{float : 'rigth'}} checkedChildren="UnHighlight" unCheckedChildren="Highlight" defaultChecked onClick = {this.switchChange} />
            { switchChange == 'UnHighlight' &&  <p onClick={this.getSelected} > <Markup content={textToHighlight} /> </p> }
            <p></p>
            <React.Fragment>
            { switchChange == 'Highlight' && <Highlighter
               highlightClassName={switchChange == 'UnHighlight' ? "unHighlightClass" : "highlightClass"}
               unhighlightClassName="unHighlightClass"
               //searchWords={ ["so", "put", "paper", "the" , "mind"] }
               autoEscape={true}
               caseSensitive = {false}
               findChunks={switchChange == 'UnHighlight' ? [] : this.sendSelected}
               textToHighlight= {replcedtext}
            /> }
            </React.Fragment>
         </div>
